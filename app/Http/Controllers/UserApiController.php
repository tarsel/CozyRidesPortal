...
use App\UserRequestDeliveryDetail;
...
class UserApiController extends Controller
{
    ...
    public  function  get_ads(){
        try{
            ...
            $today  =   Carbon::now();
            $checkAds =   Advertisement::where('status','active')
            ...
            if($checkAds){
                $advertisements =   Advertisement::where('status','active')
                    ...
                    ->where('to_date','>=', $today)
                    ->get();
                return response()->json(['error'  =>  false,'data' => $advertisements,'message' => '']);
            }
             else{
                 return response()->json(['error'  =>  true,'message' => trans('api.not_found'),'data' => array()]);
             }
  ...
  public function login(Request $request)
  {
    ...
    if(!$checkUser){
      return response()->json(['error' => trans('api.user.not_approved')], 500);
    }
    $credentials = request(['email', 'password']);
    if(!\Illuminate\Support\Facades\Auth::attempt($credentials)){
      return response()->json([
          'error' => true,
          'message' => trans('api.user.incorrect_login')
    ...
    public function services(Request $request) {
        $serviceList = ServiceType::where('type',$request->type)->exists();
        //$serviceList = ServiceType::all()
        if($serviceList) {
            $services = ServiceType::where('type',$request->type)->get();
	...
	public function send_request(Request $request) {
		...
		try{
            ...
            if(count($request->deliveries_detail) > 0){
                if($request->device_type == "android"){
                    $deliveries_detail  =   json_decode($request->deliveries_detail,true);

                }else{
                    $deliveries_detail  =   $request->deliveries_detail;
                }
                $deliveryDetails    =   [];
                foreach ($deliveries_detail as $deliveryDetailRequest) {
                    $deliveryDetails[] = [
                        'user_id' => Auth::user()->id,
                        'user_request_id'   => $UserRequest->id,
                        'item_to_deliver'   => $deliveryDetailRequest['item_to_deliver'],
                        'delivery_address'         => $deliveryDetailRequest['delivery_address'],
                        'receiver_name'         => $deliveryDetailRequest['receiver_name'],
                        'receiver_mobile'         => $deliveryDetailRequest['receiver_mobile'],
                        'any_instructions'         => $deliveryDetailRequest['any_instructions'],
                    ];
                }
                UserRequestDeliveryDetail::insert($deliveryDetails);
            }
